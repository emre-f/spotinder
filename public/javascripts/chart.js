function makeLineChart(chartName, dataset, xName, yObjs, axisLables) {
    ...
    chartObj.render = function () {
        ...
        for (var y  in yObjs) {
            // Colors of the lines
            yObjs[y].path = chartObj.svg.append("path").datum(chartObj.data).attr("class", `line`).attr("id", `line-for-${y}-in-${chartName}`).attr("d", yObjs[y].line).style("stroke", "rgba(51, 204, 102, 0.25)").attr("data-series", y).on("mouseover", function () {
        ...
        for (var y in yObjs) {
            ...
            // series.append('p').text(y).attr("class", "series-marker-text");
        ...
        function mousemove(event, d) {
            ...
            for (y in yObjs) {
                if (yObjs[y].name === closestVariable) {
                    ...
                    $(yObjs[y].legend._groups[0][0]).children().css("opacity", 1)
                } else {
                    yObjs[y].path.style("stroke", "rgba(51, 204, 102, 0.25)")
                    yObjs[y].tooltip.style("display", "none");

                    $(yObjs[y].legend._groups[0][0]).children().css("opacity", 0.6)        
                }
            }
